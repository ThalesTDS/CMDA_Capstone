...
class CodeAnalyzer:
    ...
    def analyze_code(code: str, identifier: str = "unknown") -> Dict[str, Any] | None:
        ...
        if not code_lines:
            ...
            print(f"File {identifier} does not contain any lines of code.")
            return None
        docstrings = CodeParser.extract_comments(code)

        # As of now we require 1 docstring to be present in the code to be evaluated
        if not docstrings:
            print(f"File {identifier} does not contain enough docstrings to be evaluated." + "Needs " + MIN_DOCSTRINGS + ". Has " + docstrings + ".")
            return None
