...
const FileDetails = () => {
    ...
    return (
        <div
            className={`p-6 ${theme === 'neon' ? 'bg-gradient-to-b from-background to-neon-primary/10' : 'bg-gradient-to-b from-background to-primary/5'}`}>
            {/* Background elements */}
            {theme === 'neon' && (
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-center mb-8">
                    <h2 className={`text-3xl font-bold ${theme === 'neon' ? 'text-accent' : ''}`}>
                        <div className="inline-block">
                            ...
                            <div
                                className={`h-1 w-full mt-2 ${theme === 'neon' ? 'bg-accent' : 'bg-primary'} rounded-full`}></div>
                        </div>
                    </h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    <div
                        className={`lg:col-span-1 ${cardClasses} p-6 ${showMetrics ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-500 delay-100`}>
                        <h3 className={`text-xl font-semibold mb-4 ${theme === 'neon' ? 'text-accent' : 'text-primary'}`}>Summary</h3>
                        <SummaryStats metrics={metrics}/>
                    </div>

                    <div
                        className={`lg:col-span-2 ${cardClasses} p-6 ${showMetrics ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-500 delay-200`}>
                        <h3 className={`text-xl font-semibold mb-4 ${theme === 'neon' ? 'text-accent' : 'text-primary'}`}>Documentation
                            Profile</h3>
                        <div className="h-64">
                            <RadarChart
                                data={metrics}
                ...
                {/* Gauge charts */}
                <div
                    className={`${cardClasses} p-6 ${showMetrics ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-500 delay-300`}>
                    <h3 className={`text-xl font-semibold mb-6 ${theme === 'neon' ? 'text-accent' : 'text-primary'}`}>Individual
                        Metrics</h3>
                ...
                {/* File code info */}
                <div
                    className={`mt-6 ${cardClasses} p-6 ${showMetrics ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-500 delay-400`}>
                    <h3 className={`text-xl font-semibold mb-4 ${theme === 'neon' ? 'text-accent' : 'text-primary'}`}>Code
                        Statistics</h3>
